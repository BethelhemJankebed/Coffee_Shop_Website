...
function updateAuthUI() {
  ...
  // Ask server for authoritative session user
  fetch("api.php?controller=auth&action=me")
      ...
      if (currentUser) {
        navExtras.innerHTML = `
                    <div class="user-controls" style="display: flex; align-items: center; gap: 15px;">
                        <span class="user-initials" title="${
                          currentUser.username
                        }">${currentUser.initials || "U"}</span>
    ...
    .catch((err) => console.error("Auth UI fetch failed", err));
}

function performLogout() {
  fetch("api.php?controller=auth&action=logout", { method: "POST" }).finally(
    () => {
...
async function getAuthUsers() {
  try {
    ...
    return [
      ...
      {
        id: 3,
        username: "Elias Tesfaye",
        email: "elias@coffee.com",
        password: "123",
        role: "user",
      },
    ];
