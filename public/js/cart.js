...
>>>>>>> 78af4aa (Refactored project to use complete MVC Architecture and OOP PHP):public/js/cart.js
...
=======
    ...
    async loadHistory() {
        try {
            const response = await fetch(`api.php?controller=orders&action=user_orders&user_id=${this.user.id}`);
            const data = await response.json();
